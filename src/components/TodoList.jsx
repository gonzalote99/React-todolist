...
const TodoList = () => {
 ...
 <input 
   ...
   />
        <button className='bg-red-500 hover:bg-red-400 text-white p-3 rounded-full' onClick={handleAdd} >
      ...
      <h3 className='font-bold rounded-full text-black bg-white py-3 px-7'>TASKS:</h3>
        {state.Tasks.length > 1 && (
        <button className='rounded-full text-black bg-white px-4 hover:text-red-500' onClick={() => dispatch({type: actions.DELETEALL})} >
          <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' strokeWidth={1.5} stroke='currentColor' className='w-6 h-6'>
          <path 
             strokeLinecap='round'
      ...
      <h3 className='mt-5'>not tasks added</h3>
      )}
      ...
      return(
        <li 
          ...
          <div className='flex gap-4'>
            {editTaskId === item.id ? (
            <button onClick={handleSave} className='text-red-500'>SAVE</button>
            ) : (
            <button className='text-green-500' onClick={() => handleEdit(item)} >
            EDIT
            </button>
            )}
            <button 
